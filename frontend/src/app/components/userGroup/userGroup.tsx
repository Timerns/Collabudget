...
export default function UserGroup({solde, ...props}: { solde: SoldeType, props?: any }) {
  return (
      <div className={"grid grid-cols-2 bg-secondary p-3"}>
        <div className={"col-span-1 font-semibold"}>
          <svg width="29" height="29" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg" className={"inline mr-3"}>
            <path
        ...
        <div className={`col-span-1 text-right ${Number(solde.solde) < 0 ? "text-red" : "text-green"}`}>
          {formatCurrency(Number(solde.solde))}
        </div>
      </div>
  )
}