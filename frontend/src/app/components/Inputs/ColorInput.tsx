...
const ColorInput = React.forwardRef<HTMLInputElement, ColorInputProps & ReturnType<UseFormRegister<any>>>((props, ref) => {
  const [colorReal, setColorReal] = useState("");

  useEffect(() => {
    setColorReal(props.color);
  }, [props.color])

  const onChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    props.onChange(e);
    setColorReal(e.target.value);
  }
  
  return (
    <Input {...props}>
      <div className="flex bg-white appearance-none text-sm border rounded w-full justify-between overflow-hidden">
        <p className="my-auto py-3 px-3">{colorReal}</p>
        <div className="w-15 h-auto m-[-6px] h-[calc(100% + 12px)] p-0 b-0 items-center justify-center flex">
          ...
          <input ref={ref} value={colorReal} onChange={onChange} onBlur={props.onBlur} name={props.name} type="color" className="bg-none h-full cursor-pointer" id={props.name} placeholder={props.placeholder} />
        </div>
      </div>
    </Input>
  )
